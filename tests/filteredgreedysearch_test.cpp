...
Node* createNode(unsigned int id, const vector<float>& coords, const vector<Node*>& neighbors) {
    Node* node = new Node();
    node->id = id;
    node->coords = coords;
...
void test_small_dataset_single_filter() {
    ...
    // Define the query filter (only nodes with first coordinate in {1.0, 3.0})
    ...
    TEST_CHECK(result[0]->id == 1);  // Node 1 should match the filter (first coordinate = 1.0)
    TEST_CHECK(result[1]->id == 4);  // Node 4 should also match the filter (first coordinate = 1.0)
}
...
void test_large_dataset() {
    ...
    // Define query node
    Node query;
    query.coords = {50.0, 1.0, 2.0}; // Closest nodes should have IDs near 50
    ...
    for (Node* node : result) {
        TEST_CHECK(static_cast<int>(node->coords[0]) % 2 == 0); // All results must satisfy the filter
    }
}
